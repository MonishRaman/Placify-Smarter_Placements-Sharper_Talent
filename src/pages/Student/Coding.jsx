...
const CodingPractice = () => {
  ...
  const languages = [
    ...
    { id: "python", name: "Python", icon: "🐍", monacoId: "python" },
    { id: "java", name: "Java", icon: "☕", monacoId: "java" },
  ];
  ...
  useEffect(() => {
    if (selectedProblem) {
      const newCode = selectedProblem.starterCode[selectedLanguage] || "";
      setCode(newCode);
      setTestResults(null);
  ...
  }, [selectedProblem, selectedLanguage, isEditorReady]);
  ...
  useEffect(() => {
    if (editorRef.current && isEditorReady) {
      const monaco = editorRef.current.getModel()?.getModeId
        ? window.monaco
        : null;
      if (monaco) {
  ...
  const getLanguageForMonaco = (langId) => {
  ...
  const runCode = async () => {
    ...
    // Mock test results
    const passedTests =
      Math.floor(Math.random() * selectedProblem.testCases.length) + 1;
            ...
            {filteredProblems.map((problem) => (
              ...
              >
                ...
                <div className="flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
                  <span
                    className={`px-2 py-1 rounded-full ${getDifficultyBg(
                      problem.difficulty
                    )}`}
                  >
                    {problem.difficulty}
              ...
              <div className="flex-1 flex">
                ...
                <div className="w-1/2 flex flex-col">
                  ...
                  <div className="flex-1 relative border-l border-gray-200 dark:border-gray-700">
                    ...
                    {/* Editor overlay with shortcuts hint */}
                    {!isEditorReady && (
                      <div
                    ...
                    {/* Shortcuts hint */}
                    {isEditorReady && (
                      <div className="absolute bottom-4 right-4 bg-black/80 dark:bg-gray-800/90 backdrop-blur-sm text-white text-xs px-3 py-2 rounded-lg border border-gray-600 dark:border-gray-500 shadow-lg">
                        <div className="flex items-center gap-4">
                          <span className="flex items-center gap-1">
                            <kbd className="px-1 py-0.5 bg-gray-700 dark:bg-gray-600 rounded text-xs">
                              Ctrl
                            </kbd>
                            <span>+</span>
                            <kbd className="px-1 py-0.5 bg-gray-700 dark:bg-gray-600 rounded text-xs">
                      ...
                      {testResults.success && (
                        <div className="mt-4 p-4 bg-gradient-to-r from-green-500 to-emerald-600 rounded-xl text-white">
                          ...
                          <p className="mt-2 text-sm opacity-90">
                            You've successfully solved this problem! Try the
                            next challenge.
                          </p>
                        </div>
                      )}
